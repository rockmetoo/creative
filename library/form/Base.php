	...
	abstract class Base{
		public function configure(array $properties = null){
	        if(!empty($properties)){
				...
				/*
				 * The property_reference lookup array is created so that properties
				 * can be set case-insensitively
				 * */
	            $available = array_keys(get_class_vars($class));
	            $property_reference = array();
	            foreach($available as $property) $property_reference[strtolower($property)] = $property;

				/*
				 * The method reference lookup array is created so that "set"
				 * methods can be called case-insensitively
				 * */
